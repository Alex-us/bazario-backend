...
describe('authHandler', () => {
  ...
  it('should call next if authorization header is valid', async () => {
    ...
    await authHandler(mockRequest, mockResponse, nextFunction);

    expect(mockRequest.user).toEqual({ id: mockUserA._id.toString(), deviceId: mockUserA.deviceId });
  ...
  it('should return 401 if token is for not existing user', async () => {
    jest.spyOn(authService, 'findUserById').mockResolvedValueOnce(null);
