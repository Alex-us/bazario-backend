...
export const validateAccessTokenAndReturnUserData = async (accessToken: string) => {
  if (!accessToken) {
    return false;
  }
  ...
  const userData = { id: payload.id, deviceId: payload.deviceId };
  logger.info('Access token decoded', { ...userData });
  const userFromDB = await findUserById(userData.id);

  if (userFromDB) {
    logger.info('User found in DB', { ...userData, email: userFromDB.email });
    return userData;
  } else {
    logger.info('Did not found user in DB', userData);
  }
  return;
};
